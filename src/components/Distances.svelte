<script lang="ts">
	...
	let shipsInCurrentChallenge: ShipInfo[] = $derived(
		[...shipInfos.values()].filter(isInCurrentChallenge)
	);
	let shipsNotInCurrentChallenge: ShipInfo[] = $derived(
		[...shipInfos.values()].filter((shipInfo) => !isInCurrentChallenge(shipInfo))
	);

	let maxDistance = $derived(
		shipsInCurrentChallenge.reduce((max, shipInfo) => {
...
<div class="map" {onclick}>
	...
	{#each shipsInCurrentChallenge as shipInfo}
		<div
			class="ship"
			class:noLocation={hasNoLocation(shipInfo)}
			style="top: {relativeLoc(shipInfo.location).lat}px; left: {relativeLoc(shipInfo.location)
				.lon}px;"
		>
			<img src="/ship.svg" alt={shipInfo.shipName} width="20" height="20" />
			<span
				><b>{shipInfo.shipName}</b>
				{distanceInNM(shipInfo.location, destination).toFixed(2)} nm</span
			>
		</div>
	{/each}
</div>
